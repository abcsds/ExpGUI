...
def drawCross(win, dim, crossDuration):
    xLine = visual.Line(win, lineWidth=15, start=(-300/dim[0], 0), end=(300/dim[0], 0))
    xLine.draw()
    yLine = visual.Line(win, lineWidth=15,start=(0, 300/dim[1]), end=(0, -300/dim[1]))
    yLine.draw()
    win.flip()
    #core.wait(crossDuration)
    return win

def fig_update(cLinState, cState, cTrial,timing_trialSequence, \
               timing_nStates,lsl_hOutlet,timing_taskOnset, win,\
               dim, stimulus, extension, imagePres, logger):
