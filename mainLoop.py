...
def loop(path, extension, paradigm, trialNr, blkSc, taskLen, cueLen, fixC, runType, runNr, condNr, imagePres, nMaxConsecTrials, t_mean_rand_break, t_zero_trial):
...
# RUN_TYPE = 1 # Standard
    ...
    timing_trialSequence, timing_trialTimings, timing_nStates, timing_nTrials = timing_init(RUN_TYPE, condNr, trialNr,taskLen, cueLen, blkSc, fixC, nMaxConsecTrials, t_mean_rand_break, t_zero_trial)
    ...
    if loop_measure_speed:
        ...
        loop_speed = np.empty((loop_nLoopMax,1,))
        loop_speed[:] = np.nan
    ...
    # main loop
        ...
        while loop_not_stop:
            ...
            if loop_cLinState >= -1 and loop_cLinState <= timing_nStates:
                loop_cLinState, loop_newState = timing_check(timing_tStart,timing_trialTimings,loop_cLinState,loop_newState)
    
            if loop_cLinState > -1:
                loop_cState, loop_cTrial = np.unravel_index(loop_cLinState, np.shape(timing_trialTimings), order='F')
            ...
            if loop_newState:
                ...
                fig_update(loop_cLinState, loop_cState, loop_cTrial, timing_trialSequence, timing_nStates,lsl_hOutlet, timing_taskOnset, win, dim, stimulus, extension, imagePres, logger)
    ...
    saveVariables(timing_tStart,timing_trialTimings, \
