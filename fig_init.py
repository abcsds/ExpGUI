...
def ScreenSetUp():
    ...
    # Create a window
    win = visual.Window(size = dim, monitor=allMon[dispScreen],color=[-1, -1, -1], fullscr = 1, screen = dispScreen, allowGUI  = False, checkTiming=True)
...
def loadMovies(win, stimPaths):
    ...
    for i in range(0,len(stimPaths)):
        # Open all Videos
        clip = visual.MovieStim3(win, stimPaths[i],flipVert=False, flipHoriz=False, loop=False)
...
def fig_init(RUN_TYPE,path, extension):
    ...
    message = visual.TextStim(win, text="The measurement is about to start ...", color  = [1,1,1])
    message.draw()  
    win.flip()  
